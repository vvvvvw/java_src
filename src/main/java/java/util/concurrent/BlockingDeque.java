...
import java.util.*;
//BlockingDeque相当于 FIFO BlockingQueue
/**
 ...
 *BlockingDeque 方法有四种形式，使用不同的方式处理无法立即满足但在将来某一时刻
 * 可能满足的操作：第一种方式抛出异常；第二种返回一个特殊值（null 或 false，具体取决于操作）；
 * 第三种无限期阻塞当前线程，直至操作成功；
 * 第四种只阻塞给定的最大时间，然后放弃。下表中总结了这些方法：
 ...
 *像所有 BlockingQueue 一样，BlockingDeque 是线程安全的，但不允许 null 元素，
 * 并且可能有（也可能没有）容量限制。
 ...
 * BlockingDeque 实现可以直接用作 FIFO BlockingQueue。继承自 BlockingQueue 接口的方法
 * 精确地等效于下表中描述的 BlockingDeque 方法：
 * <table BORDER CELLPADDING=3 CELLSPACING=1>
