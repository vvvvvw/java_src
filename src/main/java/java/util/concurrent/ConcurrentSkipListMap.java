 ...
 * 可缩放的并发 ConcurrentNavigableMap 实现。映射可以根据键的自然顺序进行排序，也可以根据
 * 创建映射时所提供的 Comparator 进行排序，具体取决于使用的构造方法。
 ...
 * descending ones.
 * 此类实现 SkipLists 的并发变体，为 containsKey、get、put、remove 操作及其变体提供
 * 预期平均 log(n) 时间开销。多个线程可以安全地并发执行插入、移除、更新和访问操作。
 * 迭代器是弱一致 的，返回的元素将反映迭代器创建时或创建后某一时刻的映射状态。
 * 它们不 抛出 ConcurrentModificationException，可以并发处理其他操作。
 ...
 * 此类及此类视图中的方法返回的所有 Map.Entry 对，表示他们产生时的映射关系快照。它们不 支持 Entry.setValue 方法。
 ...
 * 因为这些映射的异步特性，确定元素的当前数目需要遍历元素。此外，批量操作 putAll、equals 和 clear 并不 保证能以
 * 原子方式 (atomically) 执行。例如，与 putAll 操作一起并发操作的迭代器只能查看某些附加元素。
...
内部数据存储使用了跳表（Skip Lisy），时间复杂度O(log n)，空间复杂度O(n)。
 */
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
     ...
     */
    //用来定义最底层(base-level)的头节点(初始化时放到最底层头节点的 value)
    private static final Object BASE_HEADER = new Object();

    /**
     * The topmost head index of the skiplist.
     */
    //跳表的最高层headIndex(跳表最高层的 HeadIndex，在跳表层级更新时需要随之更新)
     ...
     */
    //存储K-V数据，持有下一个节点的引用，也就是说Node是一个单向链表
    static final class Node<K,V> {
         ...
         * 创建一个新的标记节点。 标记的区别在于其值域指向自身。
         * 标记节点的key为null，这个事实在少数地方被利用，但是这并没有将标记与
         * 基本层头节点（head.node）区分开来，后者也有一个为null的key。
         */
        Node(Node<K,V> next) {
            this.key = null;
            this.value = this;
         ...
         * 通过附加标记或从前继节点取消链接来帮助删除。 这在遍历期间当遍历节点的value为null时被调用。
         * @param b predecessor 前继节点
         * @param f successor 后继节点
         */
        void helpDelete(Node<K,V> b, Node<K,V> f) {
             ...
             * 重新检查链接，可以减少帮助线程之间的CAS干扰。
             */
            if (f == next && this == b.next) {
                // 重新检查链接，可以减少帮助线程之间的CAS干扰
                if (f == null || f.value != f) // not already marked
                    //设置为标记节点（next节点改为一个指向自身的节点）
     ...
     */
    // 跳表中的索引节点，包含了右指针(right)，向下的索引(down)和节点node。
    // 注意虽然 Node和Index都有向前的指针字段，但是它们类型不同，并且处理方式也不同，
    // 所以如果在共享的抽象类中放置这些字段不能很好地实现这一点。
     ...
     */
    //表示跳表的表头，继承自Index，内部标识了当前层级level
     ...
     */
    //比较x，y；如果c为null，则使用x的自带排序方法；否则，使用c来排序
     ...
     */
    /*
    查找最底层(base-level)节点链中比给定 key 小(在“给定节点”左边)的节点，
    也就是给定 key 节点的前继节点。如果没找到，
    那么返回底层链表的头节点。在查找过程中会帮助清除已经被删除的节点。
    函数执行流程如下：
     从head节点开始按照从上到下，从左到右的顺序查找，找到right节点的 key
     值大于给定 key 的节点，然后返回 left 节点。todo 找到的Index节点在第一层，返回的是Index节点对应的Node节点
     */
        ...
        for (;;) {
            ...
            for (Index<K,V> q = head, r = q.right, d;;) {
                if (r != null) {
                    ...
                    //比较key和k
                    //todo 如果key<=k，则不将r赋值给q，因此，返回的是left节点
                    if (cpr(cmp, key, k) > 0) {
                        //继续向右循环
                        q = r;
                        r = r.right;
     ...
     * @param onlyIfAbsent if should not insert if already present 如果当前key已经存在，是否覆盖对应的value
     * @return the old value, or null if newly inserted
     */
    /*
    1.自旋查找索引位置，更新或插入给定节点（Node）元素。在这一步中也会帮助
    清除已经删除的节点。大概流程如下：
    通过findPredecessor方法（后面会分析）首先找到最底层(base-level)给定key节点的前继节点b，
    从这个节点开始向后查找合适位置插入（或更新）
    如果在查找过程中遇到已删除节点会调用helpDelete方法帮助清除节点链接，然后重复a步骤
    2.通过“掷硬币”方式决定是否更新跳表层级。在 ConcurrentSkipListMap 中这个“掷硬币”
    随机算法实现如下：生成一个随机数rnd，如果rnd为正偶数（通过最高位和最低位都不为1验证）
    才会更新Index。然后判断这个随机数rnd从低2位开始有多少连续的1，如果这个“连续数”小于
    或等于旧表层级，则不需要增加跳表层级，只需更新Index；否则的话就需要增加跳表层级（旧表层级+1），
    并且更新Index和HeadIndex。
     */
        ...
        if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
            ...
            if (level <= (max = h.level)) {
                ...
                for (;;) {
                    ...
                    if (casHead(h, newh)) {
                        h = newh;
                        //idx赋值为在之前顶层层级新增的Index节点，并将level赋值为之前的层级
            ...
            splice: for (int insertionLevel = level;;) {
                ...
                int j = h.level;
                // 查找需要清除的节点
                // q:新跳表head; r:q.right; t:在之前顶层层级新增的Index节点
     ...
     */
    /*
    doRemove是移除节点的主方法，函数逻辑如下：

    首先找到需要删除节点的前节点，如果在查找过程中发现有已经删除的节点，
    就帮助清除节点（解除节点链接）。
    在找到需要删除的节点时并不会直接移除它，而是先利用 CAS 给这个节点添加一个
    删除标识（next 节点改为一个指向自身的节点），然后再利用 CAS 解除它的链接；
    如果途中 CAS 执行失败，则调用findNode方法来清除已经删除的节点。
    最后检查head.right如果已经被移除，就调用tryReduceLevel()方法尝试对跳表进行降级操作
    （跳表只有在层级大于3时才可以降级），tryReduceLevel()源码如下:
     */
