...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     *   关闭：不接受新任务，只处理在队列中的任务
     *   STOP:     Don't accept new tasks, don't process queued tasks,
     *             and interrupt in-progress tasks
     *   停止：不获取新任务，不处理在队列中的任务
     *   TIDYING:  All tasks have terminated, workerCount is zero,
     *             the thread transitioning to state TIDYING
     *             will run the terminated() hook method
     *   整理: 所有任务都终止，工作线程数量为0，线程变为TIDYING状态，运行terminated()钩子方法
     *   TERMINATED: terminated() has completed
     *   终止：terminated()已经执行完成
     *
     ...
     * for work.
     */
    //如果为false，即使空闲核心线程也会保持存活；如果为true，核心线程空闲
    //keepAliveTime时间后关闭
    ...
    final void tryTerminate() {
        for (;;) {
            int c = ctl.get();
            //以下几个状态不能转化为TIDYING状态:
            //RUNNING状态
            //TIDYING或TERMINATED
            //SHUTDOWN状态，但是任务队列不为空
            /*
            以下状态能转换：
    ...
    //中断空闲的worker
    /*
    为了中断worker，但是中断之前需要先获取锁，这就意味着正在运行的Worker不能中断。但是上面的代码有w.tryLock()，
    那么获取不到锁就不会中断，shutdown的Interrupt只是对所有的空闲Worker（正在从workQueue中取Task，此时Worker没有加锁）
    发送中断信号。
     */
     ...
     */
    //在调用shutdown方法之后的清理工作，在这里不会有任何操作，当时在ScheduledThreadPoolExecutor中被用户取消延时任务
    void onShutdown() {
    }
     ...
     */
    //ScheduledThreadPoolExecutor使用的状态检查来保证 在shutdown以后也能运行任务
    ...
    private List<Runnable> drainQueue() {
        ...
        q.drainTo(taskList);
        //为了防止DelayQueue或者其他poll或者drainTo这种不能一下子remove所有元素的阻塞队列，将阻塞队列转化为数组再次添加
        if (!q.isEmpty()) {
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
             ...
             * 如果=SHUTDOWN，并且任务队列不为空(SHUTDOWN下增加的Worker主要用于消耗Queue中的任务)，可以加入task为null的Worker，增加消费的Worker
             */
        ...
        try {
            ...
            if (t != null) {
                //workers是HashSet线程不安全的，那么此时需要加锁，所以mainLock.lock()，也要锁住线程池状态
                final ReentrantLock mainLock = this.mainLock;
                mainLock.lock();
                try {
     ...
     */
    //回滚 worker线程 创建
    //1.从workers中去除worker线程
    //2.减少ctl中的worker数量
    //3.检查是否termination(如果线程池在创建失败，释放锁以后终止)
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        ...
        if (runStateLessThan(c, STOP)) {
            ...
            //如果为RUNNING、SHUTDOWN,则增加一个新的worker
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            /*
            标识着当前Worker超时是否要退出。wc > corePoolSize时需要减小空闲的Worker数，那么timed为true；但是wc <= corePoolSize时，
            allowCoreThreadTimeOut为false,则不能减小核心线程数，timed为false。timedOut初始为false，如果timed为true那么使用poll取任务。如果正常返回，那么返回取到的task。
            如果超时，证明worker空闲，同时worker超过了corePoolSize，需要删除。返回null。则 timedOut = true。此时循环到
            wc <= maximumPoolSize && ! (timedOut && timed)时，减小worker数，并返回null，导致worker退出。如果线程数<= corePoolSize，
            那么此时调用 workQueue.take()，没有线程获取到时将一直阻塞，知道获取到线程或者中断，关于中断后面Shutdown的时候会说。
             */
            ...
            try {
                //如果当前Worker超时退出的话，则使用poll(keepAliveTime, TimeUnit.NANOSECONDS)；否则使用take()阻塞直到获取到任务
                Runnable r = timed ?
     ...
     * //todo ? 发生异常，worker退出，怎么创建新的worker
    ...
    final void runWorker(Worker w) {
        ...
        //标识线程是不是异常终止的，为true表示是异常终止的，为false表示是任务执行完成终止的
        ...
        try {
            ...
            while (task != null || (task = getTask()) != null) {
                ...
                //如果线程池当前状态至少为STOP,则保证worker线程 interrupted 状态置为true；
                //如果当前状态不为STOP，则保证worker线程 interrupted 状态置为false；
                //第二种情况需要重复检查，防止当调用shutdownNow方法后清除了 interrupted状态
...
RejectedExecutionHandler 线程拒绝策略。
    ...
    public void execute(Runnable command) {
         ...
         * 1.如果当前的线程数小于核心线程池的大小，根据现有的线程作为第一个Worker运行的线程，
