 ...
 * implies there are no waiting threads).
 * todo (Note that the non-blocking {@link ReadLock#tryLock()} and {@link WriteLock#tryLock()} methods
 ...
 * </ul>
 *
 * <p>todo Serialization of this class behaves in the same way as built-in
 ...
 */
/*
ReentrantReadWriteLock维护了一对相关的锁：共享锁readLock和独占锁writeLock。共享锁readLock用于读操作，
能同时被多个线程获取；独占锁writeLock用于写入操作，只能被一个线程持有。
 */
public class ReentrantReadWriteLock
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        // 最多支持65535(1<<16 -1)个写锁和65535个读锁；低16位表示写锁计数，高16位表示持有读锁计数
         ...
         */
        //最后一个获取线程的HolderCount信息,该变量主要是在如果当前线程多次获取读锁时，
        //减少从readHolds中获取HoldCounter的次数
        /**
         * 最近一个成功获取读锁的线程的计数。这省却了ThreadLocal查找，
         * 通常情况下，下一个释放线程是最近一个获取线程。这不是 volatile 的，
         * 因为它仅用于启发式的，线程进行缓存也是可以的
         ...
         */
        //firstReader与firstReadHoldCount保存第一个获取读锁的线程，
        // 也就是readHolds中并不会保存第一个获取读锁的线程
        ...
        protected final boolean tryRelease(int releases) {
            ...
            if (free)
                //exclusiveOwnerThread既不是volatile也不是cas的，cas语义
        ...
        protected final boolean tryReleaseShared(int unused) {
            Thread current = Thread.currentThread(); //获取当前线程，即释放共享锁的线程
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            if (exclusiveCount(c) != 0 &&
                ...
                //写锁被占用，且不是由当前线程持有，返回-1
                return -1;
            //执行到这里表明：写锁可用，或者写锁由当前线程持有
            //获得读锁的数量
            int r = sharedCount(c);
            /** 如果不用阻塞，且没有溢出，则使用CAS修改状态，并且修改成功 */
            if (!readerShouldBlock() && //如果在尝试获取锁时不需要阻塞等待（由公平性决定）
                    // 读锁数量还没达到最大值
                r < MAX_COUNT &&
                    ////修改高16位的状态，所以要加上2^16
                ...
                if (r == 0) { // 是 firstReader，计数不会放入 readHolds
                    ...
                    //上面判断完，到了这步，cachedHoldCounter已经是当前线程的计数值了
                    // rh.count ==0，重新设置
                    else if (rh.count == 0)
                        //将当前线程获取读锁的次数+1
                        readHolds.set(rh);
        ...
        final int fullTryAcquireShared(Thread current) {
            ...
            for (;;) {
                ...
                if (exclusiveCount(c) != 0) {
                    ...
                    // 如果“需要阻塞等待”。
                    // 1. 当“需要阻塞等待”的线程是第1个获取锁的线程的话，则继续往下执行。
                    // 2. 当“需要阻塞等待”的线程获取锁的次数=0时，则返回-1。

                    // Make sure we're not acquiring read lock reentrantly
                    // 如果当前线程如果是首个获取读锁的线程，则继续往下执行。
                    if (firstReader == current) {
                        // assert firstReaderHoldCount > 0;
                    } else {
                        //更新锁计数器
                ...
                if (compareAndSetState(c, c + SHARED_UNIT)) { //CAS更新读锁数量
                    if (sharedCount(c) == 0) { //首次获取读锁，更新firstReader和firstReaderHoldCount
                        firstReader = current;
                        firstReaderHoldCount = 1;
    ...
    static final class NonfairSync extends Sync {
        ...
        // 为了防止写线程饥饿的情况，如果AQS等待队里的第一个线程是独占的,
        // 当前读线程就阻塞。
        final boolean readerShouldBlock() {
             ...
             */
         /* 作为一个启发用于避免写线程饥饿，如果写线程出现在等待队列的头部则阻塞，
         */
