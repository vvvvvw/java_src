 ...
 */
/*
创建了一个线程队列（链表）等待获取锁。这个队列的插入操作是无锁的，
但是在插入之后，每个线程都会自旋或者让步从而被当前占有锁的线程锁阻塞。
释放锁的线程采用unsafe.park()/unpark()机制，够唤醒下一个在队列中等待的线程，从而执行了严格的优先级。
 */
/*
StampedLock是JDK1.8新增的读写锁，跟其他同步锁不同，它并不是由AQS实现的。它是一个基于能力(capability-based)的锁，
提供了三种模式的读写控制，并且内部实现了自己的同步等待队列
 */
/*
StampedLock 是不可重入的，所以在锁的内部不能调用其他尝试重复获取锁的方法。一个stamp如果在
很长时间都没有使用或验证，在很长一段时间之后可能就会验证失败。StampedLocks是可序列化的，
但是反序列化后变为初始的非锁定状态，所以在远程锁定中是不安全的。
 */
public class StampedLock implements java.io.Serializable {
    ...
    //写锁 解锁 则state+=WBIT todo 写锁会溢出，可能获取乐观读锁时，写锁加锁解锁直到循环一遍重新变为原始值（不太可能，long类型循环次数太多）
    ...
    static final class WNode {
        volatile WNode prev;
        volatile WNode next;
        volatile WNode cowait;    // list of linked readers 获取读锁的进程节点
    ...
    public long tryOptimisticRead() {
        long s;
        return (((s = state) & WBIT) == 0L) /*没有写锁*/ ? (s & SBITS) : 0L;
    }
     ...
     */
    //验证stamp，如果在锁发出给定的stamp之后写锁没有被获取，或者给定stamp是当前已经获取的锁stamp，则返回true。
    // 一般用在乐观读锁中，用于判断是否可继续获取读锁。
    ...
    private long acquireWrite(boolean interruptible, long deadline) {
        ...
        for (int spins = -1;;) { // spin while enqueuing
            ...
            else if ((p = wtail) == null) { // initialize queue wtail为空，表示队列还没有初始化
