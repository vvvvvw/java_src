 ...
 * 队列的头部 是队列中时间最长的元素。队列的尾部 是队列中时间最短的元素。新的元素插入
 * 到队列的尾部，队列获取操作从队列头部获得元素。当多个线程共享访问一个公共 collection 时，
 ...
 * 该算法基于 Maged M. Michael 和 Michael L. Scott 合著的
 ...
 * 需要小心的是，与大多数 collection 不同，size 方法不是 一个固定时间操作。
 ...
 * 内存一致性效果：当存在其他并发 collection 时，将对象放入 ConcurrentLinkedQueue 之前的线程中的操作 happen-before
 * 随后通过另一线程从 ConcurrentLinkedQueue 访问或移除该元素的操作。
...
public class ConcurrentLinkedQueue<E> extends AbstractQueue<E>
    ...
    */
     ...
     */
    //非哨兵节点（todo tail不一定永远是尾节点，但是最终是尾节点）
     ...
     */
    /*
    入队主要做两件事情，第一是将入队节点设置成当前队列尾节点的下一个节点。
    第二是更新tail节点，在入队列前如果tail节点的next节点不为空，
    则将入队节点设置成tail节点，如果tail节点的next节点为空，
    则将入队节点设置成tail的next节点，所以tail节点不总是尾节点。
     */
    ...
    public E poll() {
        ...
        for (;;) {
            ...
            for (Node<E> h = head, p = h, q;;) {
                ...
                //p为自链接节点，重新获取head循环，重新跳到restartFromHead来进行出队
                else if (p == q)
