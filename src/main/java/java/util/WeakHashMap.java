 ...
 */
//WeakHashMap在使用上和HashMap类型，都是Hash + 链表解决冲突，唯一不同点在于前者的Key是使用虚引用来实现的，
// 即当进行垃圾回收的时候，就是被回收掉，此时WeakHashMap会在下次操作的时候，根据被回收掉的Key，从Map里面移除掉。
public class WeakHashMap<K,V>
     ...
     * field as the key.
     */
    /* 这里采用了集成WeakReference而不是直接使用，是因为当被回收的时候，具体的Key是不知道的，这里需要往WeakReference额外
    加入一些属性，以便在被回收后通知时，能够定位到具体的Key/value */
